/**
 * Displays WV data on the DOM using a geoAlbers projection. Requires {@link https://d3js.org/d3.v4.min.js d3}. 
 * 
 ...
 class WVCensusMap {
    ...
    constructor(keymodeler,geodata, censusdata, container, tooltipShowEventType ="mouseenter touchstart",tooltipHideEventType="mouseleave touchend", sequentialmultihue=d3.interpolateInferno, guessFormatting=true,resize=false ) {
        ...
        this.numberFormatters = {
            'currency change': new Intl.NumberFormat('en', {style:'currency', currency:'USD', signDisplay:'exceptZero'}),
            'currency static': new Intl.NumberFormat('en', {style:'currency', currency:'USD'}),
            'percent change': new Intl.NumberFormat('en', {style:'percent',minimumSignificantDigits:4, maximumSignificantDigits:4, signDisplay:'exceptZero'}),
            'percent static': new Intl.NumberFormat('en', {style:'percent', minimumSignificantDigits:4, maximumSignificantDigits:4 }),
            'number': new Intl.NumberFormat('en'),
            'number change': new Intl.NumberFormat('en', {signDisplay:'exceptZero'})
        };
        this.currentNumberFormatter = this.numberFormatters['number'];
        this.renderMap();
        let that = this;
    ...
    renderData() {
        ...
        stateKeys.forEach( (key)=> {
            try {
            let val = this.keymodeler.getPropVal(this.censusdata[key]['Data']);
            datarange.push(val);
            } catch(e) {
                console.error(`didnt get value ${e}`)
            }
         });
        // set number formatter
        if (this.guessFormatting) {
            let kmlength = this.keymodeler.selectedValues.length;
            let lastSelectedValue = this.keymodeler.selectedValues[kmlength-1];
            console.log(lastSelectedValue);
            let changing = false;
            let dollars = false;
            let percent = false;
            if(/[iI]ncrease|[dD]ecrease|[cC]hange/.test(lastSelectedValue)) {
                changing = true;
            }
            if(/\$|(usd)|USD|Usd|dollars|Dollars/.test(lastSelectedValue)) {
                dollars = true;
            }
            else if(/\%|percent|Percent/.test(lastSelectedValue)) {
                percent = true;
            }
            if(changing) {
                if(dollars) {
                    this.currentNumberFormatter = this.numberFormatters['currency change'];
                }
                else if(percent) {
                    this.currentNumberFormatter = this.numberFormatters['percent change'];
                }
                else {
                    this.currentNumberFormatter = this.numberFormatters['number change'];
                }
            }
            else {
                if(dollars) {
                    this.currentNumberFormatter = this.numberFormatters['currency static'];
                }
                else if(percent) {
                    this.currentNumberFormatter = this.numberFormatters['percent static'];
                }
                else {
                    this.currentNumberFormatter = this.numberFormatters['number'];
                }
        ...
        this.legendSVG.attr('color','black')
        ...
        this.pathGroup.selectAll("path")
            .attr('fill',(d) => {
                let name = parseInt(d.properties.COUNTY);
                try {
                    let obj = this.censusdata[name]['Data'];
        ...
        this.extremaDiv.append('div')
            ...
            .text(this.censusdata[minState]['RegionName']);
        this.extremaDiv.append('div')
        ...
        this.extremaDiv.append('div')
            ...
            .text(this.censusdata[maxState]['RegionName']);
        this.extremaDiv.append('div')
     ...
     * @param {string} [type=currency] type The type to process it to
     */
    formatVal(val, type='round number') {
        let test = parseFloat(val);
        if(isNaN(test)) {
            return val;
        }
        else {
            return this.currentNumberFormatter.format(val)
        }
    }
    /**
    ...
    tooltipShow(event,feature) { 
        //console.log(feature.target.__data__.properties);
        let properties = feature.target.__data__.properties;
        let name = properties.NAME;
        let area = properties.CENSUSAREA;
        this.tooltip.style('opacity',1)
        this.tooltip.select('#usc-tool-state')
            .text(name);
        let val = null;
        try {
            let census_data_for_county = this.censusdata[parseInt(properties.COUNTY)]['Data'];
            val = this.keymodeler.getPropVal(census_data_for_county);
        }
        catch (e) {
            console.error(`error with tooltip hover, value probably not found in data: ${e}`);
            val = 'unknown'
        }
